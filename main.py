...
class Hotel:
    ...
    def available(self):
        """Check to see if hotel is available"""
...
try:
    if hotel.available():
        ...
        if credit_card.validate(expiration="12/26", holder="JOHN SMITH", cvc="123"):
            password = input("Please provide your credit card password: ")
            if credit_card.authenticate(given_password=password):
                ...
                spa_reservation_ticket=SpaReservation(customer_name=name, hotel_object=hotel)
                if spa_booking == "yes":
                    print(spa_reservation_ticket.generate())
                else:
                    print("No problem! We hope you have a wonderful stay.")
            else:
                print("Your credit card authentication failed.")
        else:
            print("There was a problem with your payment")
    else:
        print("Hotel is not free.")
except ValueError:
    print("Please make sure to select the correct Hotel ID")